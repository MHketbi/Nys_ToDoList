
NysToDoListDB = {
	["global"] = {
		["tuto_progression"] = 6,
	},
	["profileKeys"] = {
	},
	["profiles"] = {
		["Default"] = {
			["itemsList"] = {
				["Other"] = {
					["World Quests"] = {
						["tabName"] = "Daily",
						["checked"] = false,
						["description"] = "get anima, conduits and gear from WQ",
					},
				},
				["Torghast"] = {
					["Wing 1"] = {
						["tabName"] = "Weekly",
						["checked"] = false,
					},
					["Wing 2"] = {
						["tabName"] = "Weekly",
						["checked"] = false,
					},
				},
				["Mandatory"] = {
					["Complete Campaign"] = {
						["tabName"] = "Daily",
						["checked"] = true,
						["description"] = "you can remove this once completed",
					},
					["Complete Covenant quest"] = {
						["tabName"] = "Daily",
						["checked"] = true,
						["description"] = "you can remove this once completed",
					},
				},
				["Covenant"] = {
					["Daily calls"] = {
						["tabName"] = "Daily",
						["checked"] = false,
						["description"] = "Emissary like daily calls for rewards",
					},
					["Adventures"] = {
						["tabName"] = "Daily",
						["checked"] = false,
						["description"] = "do your table missions",
					},
					["Treasures"] = {
						["tabName"] = "Daily",
						["checked"] = false,
						["description"] = "For Reservoir Anima and other items",
					},
					["Reservoir Anima"] = {
						["tabName"] = "Daily",
						["checked"] = false,
						["description"] = "From Dungeons and WQ",
					},
					["Rares"] = {
						["tabName"] = "Daily",
						["checked"] = false,
						["description"] = "For Reservoir Anima and other items",
					},
				},
				["The Vault"] = {
					["M+ (1 time)"] = {
						["tabName"] = "Weekly",
						["checked"] = false,
					},
					["M+ (10 times)"] = {
						["tabName"] = "Weekly",
						["checked"] = false,
					},
					["Rated-PVP (1250 Honor)"] = {
						["tabName"] = "Weekly",
						["checked"] = false,
					},
					["Raid (3 bosses)"] = {
						["tabName"] = "Weekly",
						["checked"] = false,
					},
					["Rated-PVP (6250 Honor)"] = {
						["tabName"] = "Weekly",
						["checked"] = false,
					},
					["M+ (4 times)"] = {
						["tabName"] = "Weekly",
						["checked"] = false,
					},
					["Raid (10 bosses)"] = {
						["tabName"] = "Weekly",
						["checked"] = false,
					},
					["Rated-PVP (2500 Honor)"] = {
						["tabName"] = "Weekly",
						["checked"] = false,
					},
					["Raid (7 bosses)"] = {
						["tabName"] = "Weekly",
						["checked"] = false,
					},
				},
				["Mythic0"] = {
					["Necrotic Wake"] = {
						["tabName"] = "Weekly",
						["checked"] = true,
					},
					["De Other Side"] = {
						["tabName"] = "Weekly",
						["checked"] = true,
					},
					["Halls of Atonement"] = {
						["tabName"] = "Weekly",
						["checked"] = false,
					},
					["Theatre of Pain"] = {
						["tabName"] = "Weekly",
						["checked"] = true,
					},
					["Mists of Tirna Scithe"] = {
						["tabName"] = "Weekly",
						["checked"] = true,
					},
					["Sanguine Depths"] = {
						["tabName"] = "Weekly",
						["checked"] = false,
					},
					["Spires of Ascension"] = {
						["tabName"] = "Weekly",
						["checked"] = false,
					},
					["Plaguefall"] = {
						["tabName"] = "Weekly",
						["checked"] = true,
					},
				},
				["Legendary/Conduit"] = {
					["Farm Conduit"] = {
						["tabName"] = "Daily",
						["checked"] = false,
						["description"] = "know where your best conduit come from and farm that content",
					},
					["Legendary Recipe"] = {
						["tabName"] = "Daily",
						["checked"] = false,
						["description"] = "know where the recipe you want comes from and farm it",
					},
				},
				["Quests"] = {
					["Weekly PVP/Dungeon 3"] = {
						["tabName"] = "Weekly",
						["checked"] = false,
						["description"] = "from Oribos",
					},
					["Weekly PVP/Dungeon 2"] = {
						["tabName"] = "Weekly",
						["checked"] = false,
						["description"] = "from Oribos",
					},
					["Return Lost Souls"] = {
						["tabName"] = "Weekly",
						["checked"] = false,
						["description"] = "from covenant",
					},
					["Replenish the Reservoir"] = {
						["tabName"] = "Weekly",
						["checked"] = false,
						["description"] = "from covenant",
					},
					["Weekly PVP/Dungeon 1"] = {
						["tabName"] = "Weekly",
						["checked"] = false,
						["description"] = "from Oribos",
					},
				},
				["Maw"] = {
					["Stygia"] = {
						["tabName"] = "Daily",
						["checked"] = false,
						["description"] = "Currency for socket and Torghast upgrades",
					},
					["Daily Quest 1"] = {
						["tabName"] = "Daily",
						["checked"] = false,
						["description"] = "on the maw map you should see a blue quest mark",
					},
					["Reputation"] = {
						["tabName"] = "Daily",
						["checked"] = false,
						["description"] = "to open more Torghast upgrades",
					},
					["Eye of the Jailor Locked"] = {
						["tabName"] = "Daily",
						["checked"] = false,
						["description"] = "everyday while farming in the maw you are limited by the eye of the jailor",
					},
					["Daily Quest 2"] = {
						["tabName"] = "Daily",
						["checked"] = false,
						["description"] = "on the maw map you should see a blue quest mark",
					},
				},
			},
			["lastLoadedTab"] = "ToDoListUIFrameTab3",
			["dailyHour"] = 8,
			["frameSize"] = {
				["height"] = 600,
				["width"] = 400.0001831054688,
			},
			["autoReset"] = {
				["Weekly"] = 1606881599,
				["Daily"] = 1606363199,
			},
			["undoTable"] = {
				{
					["tabName"] = "Daily",
					["itemName"] = "Wing 1",
					["checked"] = false,
					["catName"] = "Torghast",
				}, -- [1]
				{
					["tabName"] = "Daily",
					["itemName"] = "Wing 2",
					["checked"] = false,
					["catName"] = "Torghast",
				}, -- [2]
				{
					["tabName"] = "Daily",
					["itemName"] = "Farm Anima",
					["checked"] = false,
					["catName"] = "Covenant",
				}, -- [3]
				{
					["tabName"] = "Daily",
					["itemName"] = "Farm Rares",
					["checked"] = false,
					["catName"] = "Covenant",
				}, -- [4]
				{
					["tabName"] = "Daily",
					["itemName"] = "Farm Treasures",
					["checked"] = false,
					["catName"] = "Covenant",
				}, -- [5]
				{
					["tabName"] = "Weekly",
					["itemName"] = "2x M+ Dungeon",
					["checked"] = false,
					["catName"] = "the Vault",
				}, -- [6]
				{
					["tabName"] = "Weekly",
					["itemName"] = "15x M+ Dungeon",
					["checked"] = false,
					["catName"] = "the Vault",
				}, -- [7]
				{
					["tabName"] = "Weekly",
					["itemName"] = "10x M+ Dungeon",
					["checked"] = false,
					["catName"] = "the Vault",
				}, -- [8]
				{
					["tabName"] = "Weekly",
					["itemName"] = "1x M+ Dungeon",
					["checked"] = false,
					["catName"] = "the Vault",
				}, -- [9]
				{
					["tabName"] = "Weekly",
					["itemName"] = "3x Raid bosses",
					["checked"] = false,
					["catName"] = "the Vault",
				}, -- [10]
				{
					["tabName"] = "Weekly",
					["itemName"] = "4x M+ Dungeon",
					["checked"] = false,
					["catName"] = "the Vault",
				}, -- [11]
				{
					["tabName"] = "Weekly",
					["itemName"] = "7x Raid bosses",
					["checked"] = false,
					["catName"] = "the Vault",
				}, -- [12]
				{
					["tabName"] = "Weekly",
					["itemName"] = "Mists of Tirna Scithe",
					["checked"] = false,
					["catName"] = "Mythic0",
				}, -- [13]
				{
					["tabName"] = "Weekly",
					["itemName"] = "Replenish the Reservoir ",
					["checked"] = false,
					["catName"] = "Quest",
				}, -- [14]
				{
					["tabName"] = "Weekly",
					["itemName"] = "Return Lost Souls ",
					["checked"] = false,
					["catName"] = "Quest",
				}, -- [15]
				{
					["tabName"] = "Weekly",
					["itemName"] = "heroic dungeon weekly",
					["checked"] = false,
					["catName"] = "Quests",
				}, -- [16]
				{
					["tabName"] = "Weekly",
					["itemName"] = "normal dungeon weekly",
					["checked"] = false,
					["catName"] = "Quests",
				}, -- [17]
				{
					["tabName"] = "Weekly",
					["itemName"] = "pvp weekly 1",
					["checked"] = false,
					["catName"] = "Quests",
				}, -- [18]
				{
					["tabName"] = "Weekly",
					["itemName"] = "pvp weekly 2",
					["checked"] = false,
					["catName"] = "Quests",
				}, -- [19]
			},
			["framePos"] = {
				["xOffset"] = 456.4074401855469,
				["yOffset"] = 81.96298217773438,
			},
		},
	},
}
